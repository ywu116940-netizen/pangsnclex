...
async function start(id,count){
  ...
  practiceError='';generationBusy=true;quiz=null;go('practice');
  try{
    const res=await fetch(isDemo?'/api/quiz/demo':'/api/quiz/generate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({count:isDemo?1:requested,sources:selected.map(m=>({id:m.id,name:m.name,text:m.source}))})});
...
function rationale(q){
  ...
  return `<section class="rationale" aria-live="polite"><h3>${quiz.selected===q.correct?'✓ That’s correct.':'Let’s work through this.'}</h3><p><strong>Correct answer: ${'ABCD'[q.correct]}</strong> · ${esc(q.options[q.correct])}</p><p>${esc(q.correctRationale)}</p>${evidence([...q.stemEvidence,...(q.optionRationales.find(r=>r.optionIndex===q.correct)?.evidence||[])].filter((e,i,all)=>all.findIndex(other=>other.sourceId===e.sourceId&&other.quote===e.quote)===i))}<details open><summary>Why the other choices don’t fit</summary><ul>${q.optionRationales.filter(r=>r.optionIndex!==q.correct).map(r=>`<li><strong>${'ABCD'[r.optionIndex]}. ${esc(q.options[r.optionIndex])}</strong><p>${esc(r.explanation)}</p>${evidence(r.evidence)}</li>`).join('')}</ul></details><p class="form-note">${q.demo?'Authored practice example from the skin-condition text you supplied.':'AI-generated study practice. Review the cited material when checking clinical details.'}</p></section>`;
}
